...
# ace がすでにインストールされている場合は何もしない
if command -v ace >/dev/null 2>&1; then
  echo "ace is already installed. Skipping feature install."
  exit 0
fi
...
if [ "${VERSION}" = "latest" ]; then
  VERSION=$(
    download https://api.github.com/repos/kurusugawa-computer/ace/releases/latest \
    | grep '"tag_name":' \
    | sed -E 's/.*"([^"]+)".*/\1/'
  )
  if [ -z "${VERSION}" ]; then
...
# ace をインストール
download_url="https://github.com/kurusugawa-computer/ace/releases/download/${VERSION}/ace_linux_${arch}.tar.gz"
echo download ace: "${download_url}"
download "${download_url}" | tar -xz -C /tmp
mkdir -p /usr/local/bin
mv /tmp/ace_linux_${arch}/ace /usr/local/bin/ace
rm -rf /tmp/ace_linux_${arch}/ace
